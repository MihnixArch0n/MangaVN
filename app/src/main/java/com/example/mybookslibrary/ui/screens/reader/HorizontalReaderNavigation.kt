...
internal fun evaluateHorizontalTap(
    ...
    return when {
        tapRatio < LEFT_ZONE_END_RATIO -> when (readingMode) {
        ...
        tapRatio >= RIGHT_ZONE_START_RATIO -> when (readingMode) {
...
internal fun calculateHorizontalTargetPage(
    ...
    val pageDelta = when (action) {
        ReaderTapAction.NEXT_PAGE -> 1
        ReaderTapAction.PREVIOUS_PAGE -> -1
        ReaderTapAction.TOGGLE_OVERLAY,
        ReaderTapAction.NONE -> return null
    }
    return (targetPage + pageDelta).coerceIn(0, lastPageIndex)
}
